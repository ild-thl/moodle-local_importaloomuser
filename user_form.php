...
//Form with 
class admin_uploadaloomuser_form1 extends moodleform {
    function definition () {
        $mform = $this->_form;

        $mform->addElement('header', 'settingsheader', get_string('upload'));
        $choices = core_text::get_encodings();
        $mform->addElement('select', 'encoding', get_string('encoding', 'local_importaloomuser'), $choices);
        $mform->setDefault('encoding', 'UTF-8');

        $choices = array('10'=>10, '20'=>20, '100'=>100, '1000'=>1000, '100000'=>100000);
        $mform->addElement('select', 'previewrows', get_string('rowpreviewnum', 'local_importaloomuser'), $choices);
        $mform->setType('previewrows', PARAM_INT);

        $this->add_action_buttons(false, get_string('uploadusers', 'local_importaloomuser'));
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_uploadaloomuser_form2 extends moodleform {
    function definition () {
        global $CFG, $USER;
        ...
        $choices = array(UU_USER_ADDNEW     => get_string('uuoptype_addnew', 'local_importaloomuser'),
                         UU_USER_ADDINC     => get_string('uuoptype_addinc', 'local_importaloomuser'),
                         UU_USER_ADD_UPDATE => get_string('uuoptype_addupdate', 'local_importaloomuser'),
                         UU_USER_UPDATE     => get_string('uuoptype_update', 'local_importaloomuser'));
        $mform->addElement('select', 'uutype', get_string('uuoptype', 'local_importaloomuser'), $choices);

        $choices = array(0 => get_string('infilefield', 'auth'), 1 => get_string('createpasswordifneeded', 'auth'));
        $mform->addElement('select', 'uupasswordnew', get_string('uupasswordnew', 'local_importaloomuser'), $choices);
        $mform->setDefault('uupasswordnew', 1);
        $mform->hideIf('uupasswordnew', 'uutype', 'eq', UU_USER_UPDATE);

        $choices = array(UU_UPDATE_NOCHANGES    => get_string('nochanges', 'local_importaloomuser'),
                         UU_UPDATE_FILEOVERRIDE => get_string('uuupdatefromfile', 'local_importaloomuser'),
                         UU_UPDATE_ALLOVERRIDE  => get_string('uuupdateall', 'local_importaloomuser'),
                         UU_UPDATE_MISSING      => get_string('uuupdatemissing', 'local_importaloomuser'));
        $mform->addElement('select', 'uuupdatetype', get_string('uuupdatetype', 'local_importaloomuser'), $choices);
        ...
        $choices = array(0 => get_string('nochanges', 'local_importaloomuser'), 1 => get_string('update'));
        $mform->addElement('select', 'uupasswordold', get_string('uupasswordold', 'local_importaloomuser'), $choices);
        ...
        $choices = array(UU_PWRESET_WEAK => get_string('usersweakpassword', 'local_importaloomuser'),
        ...
        $mform->addElement('selectyesno', 'uuallowrenames', get_string('allowrenames', 'local_importaloomuser'));
        ...
        $mform->addElement('selectyesno', 'uuallowdeletes', get_string('allowdeletes', 'local_importaloomuser'));
        ...
        $mform->addElement('selectyesno', 'uuallowsuspends', get_string('allowsuspends', 'local_importaloomuser'));
        ...
        if (!empty($CFG->allowaccountssameemail)) {
            $mform->addElement('selectyesno', 'uunoemailduplicates', get_string('uunoemailduplicates', 'local_importaloomuser'));
        ...
        $mform->addElement('selectyesno', 'uustandardusernames', get_string('uustandardusernames', 'local_importaloomuser'));
        $mform->setDefault('uustandardusernames', 1);

        $choices = array(UU_BULK_NONE    => get_string('no'),
                         UU_BULK_NEW     => get_string('uubulknew', 'local_importaloomuser'),
                         UU_BULK_UPDATED => get_string('uubulkupdated', 'local_importaloomuser'),
                         UU_BULK_ALL     => get_string('uubulkall', 'local_importaloomuser'));
        $mform->addElement('select', 'uubulk', get_string('uubulk', 'local_importaloomuser'), $choices);
        ...
        if ($showroles) {
            ...
            $mform->addElement('select', 'uulegacy1', get_string('uulegacy1role', 'local_importaloomuser'), $choices);
            ...
            $mform->addElement('select', 'uulegacy2', get_string('uulegacy2role', 'local_importaloomuser'), $choices);
            ...
            $mform->addElement('select', 'uulegacy3', get_string('uulegacy3role', 'local_importaloomuser'), $choices);
        ...
        // default values
        $mform->addElement('header', 'defaultheader', get_string('defaultvalues', 'local_importaloomuser'));

        $mform->addElement('text', 'username', get_string('uuusernametemplate', 'local_importaloomuser'), 'size="20"');
        ...
        $mform->addRule('username', get_string('requiredtemplate', 'local_importaloomuser'), 'required', null, 'client');
        ...
        $this->add_action_buttons(true, get_string('uploadusers', 'local_importaloomuser'));
    ...
    function validation($data, $files) {
        ...
        if (!empty($updatetype) && ($optype == UU_USER_ADDNEW || $optype == UU_USER_ADDINC)) {
            $errors['uuupdatetype'] = get_string('invalidupdatetype', 'local_importaloomuser');
        }
        ...
        if ($optype != UU_USER_UPDATE) {
            ...
            if (!in_array('email', $columns) and empty($data['email'])) {
                $errors['email'] = get_string('requiredtemplate', 'local_importaloomuser');
            }
        }
        return $errors;
    }
